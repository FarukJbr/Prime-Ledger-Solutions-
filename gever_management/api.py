...
def verify_auth(c:HTTPBasicCredentials=Depends(security)):
    if not (secrets.compare_digest(c.username.encode(),DASHBOARD_USER.encode()) and
            secrets.compare_digest(c.password.encode(),DASHBOARD_PASS.encode())):
        raise HTTPException(401,headers={"WWW-Authenticate":"Basic"})
    return c.username

SBH=lambda:{"apikey":SUPABASE_KEY,"Authorization":f"Bearer {SUPABASE_KEY}","Content-Type":"application/json","Prefer":"return=representation"}

async def sb_get(t,p=""):
    if not SUPABASE_URL:return[]
    try:
        async with httpx.AsyncClient() as c:
            r=await c.get(f"{SUPABASE_URL}/rest/v1/{t}{'?'+p if p else ''}",headers=SBH(),timeout=15)
            return r.json() if r.status_code==200 else []
    except:return[]

async def sb_ins(t,d):
    if not SUPABASE_URL:return{"id":"mock",**d}
    try:
        async with httpx.AsyncClient() as c:
            r=await c.post(f"{SUPABASE_URL}/rest/v1/{t}",headers=SBH(),json=d,timeout=15)
            res=r.json()
            return res[0] if isinstance(res,list)and res else(res if isinstance(res,dict) else{"id":"ok"})
    except Exception as e:return{"id":"err","e":str(e)}

AGENTS={"ceo":("דניאל כהן","מנכ\"ל","אתה דניאל כהן מנכ\"ל {co}."),"cfo":("מיכאל לוי","CFO","אתה מיכאל לוי CFO {co}."),"marketing":("נועה שפירא","מנהלת שיווק","את נועה שפירא שיווק {co}."),"sales":("רון אברהם","מנהל מכירות","אתה רון אברהם מכירות {co}."),"legal":("עו\"ד תמר גולן","יועמ\"ש","את תמר גולן יועמ\"ש {co}."),"cto":("אלון בן-דוד","CTO","אתה אלון CTO {co}."),"content":("שיר מזרחי","מנהלת תוכן","את שיר מזרחי תוכן {co}."),"pr":("גיל פרץ","מנהל יח\"צ","אתה גיל פרץ יח\"צ {co}."),"compliance":("ד\"ר ענת רוזן","קצינת ציות","את ענת רוזן ציות {co}."),"hr":("יובל כץ","מנהל HR","אתה יובל כץ HR {co}."),"customer":("ליאת דביר","מנהלת שירות","את ליאת דביר שירות לקוחות {co}.")}
DMETA={"ceo":{"name":"מנכ\"ל","icon":"👑","color":"#a78bfa"},"cfo":{"name":"כספים","icon":"💰","color":"#f5c842"},"marketing":{"name":"שיווק","icon":"📣","color":"#f87171"},"sales":{"name":"מכירות","icon":"📈","color":"#22d3a0"},"legal":{"name":"משפטי","icon":"⚖️","color":"#60a5fa"},"cto":{"name":"טכנולוגיה","icon":"💻","color":"#34d399"},"content":{"name":"תוכן","icon":"🎨","color":"#fb923c"},"pr":{"name":"יח\"צ","icon":"📢","color":"#e879f9"},"compliance":{"name":"ציות","icon":"🛡️","color":"#94a3b8"},"hr":{"name":"HR","icon":"👥","color":"#4ade80"},"customer":{"name":"שירות לקוחות","icon":"🎧","color":"#38bdf8"}}
EMPS={"ceo":[("יעל מזרחי","עוזרת מנכ\"ל"),("אסף ברק","פרויקטים"),("מיה לוין","רכזת"),("עומר שלום","אנליסט")],"cfo":[("דנה כהן","חשבת"),("רועי פלד","אנליסט"),("שרה לוי","גזברית"),("אמיר גל","תקציב")],"marketing":[("נדב ביטון","דיגיטל"),("טל שר","מעצב"),("יונית אור","תוכן"),("עידן רז","SEO")],"sales":[("ליאל דוד","מכירות"),("הילה ים","לקוחות"),("בן גבע","אנליסט"),("מור שגיא","מכירות")],"legal":[("ניר אלון","עו\"ד"),("שירה רן","פרלגל"),("גבי מור","יועץ"),("לי בן","מזכירה")],"cto":[("ירון נוי","Full Stack"),("הדר עם","DevOps"),("ליר שן","Backend"),("כרמל אל","UX")],"content":[("אביב כץ","צלם"),("ניל שר","סושיאל"),("עלמא פז","תוכן"),("יאיר אף","YouTube")],"pr":[("הילה דן","דוברת"),("רן שם","יחצ\"ן"),("שי לם","אירועים"),("מאיה ון","תקשורת")],"compliance":[("ורד נץ","ציות"),("תמר גן","מבקרת"),("אלי קם","סיכונים"),("רינה שן","רגולציה")],"hr":[("נועם בר","גיוס"),("שלי גז","רווחה"),("עמית לז","הכשרות"),("ציפי רם","ייעוץ")],"customer":[("דור כהן","שירות"),("עינת שמ","שירות"),("אלון בר","תלונות"),("מרים כ","שירות")]}

async def claude(role,msg,ctx=""):
    if not ANTHROPIC_API_KEY:return f"[{AGENTS.get(role,('AI','',''))[0]}]: API key לא מוגדר"
    n,t,sys=AGENTS.get(role,("AI","","אתה עוזר."))
    system=sys.replace("{co}",COMPANY_NAME)
    if ctx:system+=f"\n\nהקשר:\n{ctx}"
    try:
        async with httpx.AsyncClient() as c:
            r=await c.post("https://api.anthropic.com/v1/messages",
                headers={"x-api-key":ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01","content-type":"application/json"},
                json={"model":"claude-opus-4-6","max_tokens":1500,"system":system,"messages":[{"role":"user","content":msg}]},timeout=40)
            return r.json()["content"][0]["text"] if r.status_code==200 else f"שגיאה {r.status_code}"
    except Exception as e:return f"שגיאה: {e}"

@app.get("/api/health")
async def health():
    return {"status":"ok","anthropic":bool(ANTHROPIC_API_KEY),"supabase":bool(SUPABASE_URL and SUPABASE_KEY),"facebook":bool(META_PAGE_TOKEN),"instagram":bool(INSTAGRAM_ID),"whatsapp":bool(WHATSAPP_TOKEN)}
...
async def stats(u=Depends(verify_auth)):
    ...
    d=await sb_get("tasks","select=count&status=eq.done")
    def cnt(x):return x[0].get("count",0) if isinstance(x,list)and x and isinstance(x[0],dict) else 0
    return{"tasks_total":cnt(t),"tasks_done":cnt(d),"tasks_inprog":0,"tasks_pending":0,"departments":len(DMETA),"employees":55}

@app.get("/api/departments")
async def depts(u=Depends(verify_auth)):
    return[{"id":k,"name":v["name"],"icon":v["icon"],"color":v["color"],"head":AGENTS[k][0],"title":AGENTS[k][1]}for k,v in DMETA.items()]
