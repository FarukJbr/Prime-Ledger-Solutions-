...
@app.get("/api/verify")
async def verify_endpoint(user=Depends(verify_auth)):
...
async function login(){
  ...
  const r=await fetch('/api/verify',{headers:{Authorization:'Basic '+btoa(u+':'+p)}});
  if(r.ok){const t=btoa(u+':'+p);sessionStorage.setItem('jabr_auth',t);window.location.href='/dashboard#'+t;}
...
<script>
// Auto-auth: read from hash, sessionStorage, or redirect to login
let AUTH='';
if(location.hash&&location.hash.length>1){
  AUTH=location.hash.substring(1);
  sessionStorage.setItem('jabr_auth',AUTH);
  history.replaceState(null,null,' ');
}
if(!AUTH)AUTH=sessionStorage.getItem('jabr_auth')||'';
if(!AUTH){window.location.href='/';throw new Error('no auth');}
const AH={'Content-Type':'application/json','Authorization':'Basic '+AUTH};
...
async function login(){
  ...
  document.getElementById('err').style.display='none';
  const r=await fetch('/api/verify',{headers:{Authorization:'Basic '+btoa(u+':'+p)}});
  if(r.ok){const t=btoa(u+':'+p);sessionStorage.setItem('jabr_auth',t);document.cookie='jabr_auth='+t+';path=/;max-age=86400';window.location.href='/dashboard';}
  else{document.getElementById('err').style.display='block';btn.textContent='🔐 כניסה למערכת';}
}
</script>
</body></html>'''
...
<script>
// Auto-auth
let AUTH=sessionStorage.getItem('jabr_auth')||'';
if(!AUTH){const m=document.cookie.match(/jabr_auth=([^;]+)/);if(m)AUTH=decodeURIComponent(m[1]);}
if(!AUTH){window.location.href='/';throw new Error('no auth');}
const AH={'Content-Type':'application/json','Authorization':'Basic '+AUTH};
async function api(m,p,b){
  const o={method:m,headers:AH};if(b)o.body=JSON.stringify(b);
...
<script>
// Auto-auth: works whether entering via login page or direct URL
let AUTH=sessionStorage.getItem('jabr_auth')||'';
if(!AUTH){
  // Try to get from cookie fallback
  const m=document.cookie.match(/jabr_auth=([^;]+)/);
  if(m)AUTH=m[1];
}
